<?php
  ...
  $teamStmt = $pdo->prepare('SELECT Teams.team_id,Teams.team_name,Games.game_id,Games.next_game,Games.team_a,Games.team_b,Games.get_wildcard,Games.is_wildcard FROM Teams JOIN Games JOIN Groups WHERE (Teams.team_id=Games.team_a OR Teams.team_id=Games.team_b) AND Games.tourn_id=Groups.fk_tourn_id AND (Games.first_round=1 OR Games.is_wildcard=1) AND Groups.group_id=:gid');
